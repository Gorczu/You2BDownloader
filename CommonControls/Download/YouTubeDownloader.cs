...
namespace CommonControls.Download
{
    public class YouTubeDownloader
    {
        ...
        public class Helper
        {
            internal static string DownloadWebPage(string videoUrl)
            {
                StringBuilder responseBuilder = new StringBuilder();
                WebRequest req = WebRequest.Create(videoUrl);
                WebResponse response = req.GetResponse();
                using (TextReader reader = new StreamReader(response.GetResponseStream()))
                {
                    responseBuilder.Append(reader.ReadToEnd());
                }

                return responseBuilder.ToString();
            }

            internal static string GetTxtBtwn(string rssDoc, string v1, string v2, int v3)
            {
                return "";
            }
        }
